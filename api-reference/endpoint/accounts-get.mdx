---
title: "Get Account"
api: "GET https://api.railsinfra.com/api/v1/accounts/{id}"
description: "Retrieve a specific account by ID"
---

Retrieves detailed information for a specific account by its unique identifier.

## Path Parameters

<ParamField path="id" type="string" required>
  Unique identifier for the account (UUID format)
</ParamField>

## Response

<ResponseField name="id" type="string">
  Unique identifier for the account (UUID)
</ResponseField>

<ResponseField name="account_number" type="string">
  Auto-generated account number
</ResponseField>

<ResponseField name="account_type" type="string">
  Type of account: `checking` or `saving`
</ResponseField>

<ResponseField name="user_id" type="string">
  ID of the account owner
</ResponseField>

<ResponseField name="balance" type="number">
  Current account balance (decimal)
</ResponseField>

<ResponseField name="currency" type="string">
  Currency code (e.g., `USD`)
</ResponseField>

<ResponseField name="status" type="string">
  Account status: `active`, `suspended`, or `closed`
</ResponseField>

<ResponseField name="organization_id" type="string">
  Organization ID (if applicable)
</ResponseField>

<ResponseField name="environment" type="string">
  Environment: `production` or `sandbox`
</ResponseField>

<ResponseField name="admin_user_id" type="string">
  Admin user ID (if applicable)
</ResponseField>

<ResponseField name="user_role" type="string">
  User role (if applicable)
</ResponseField>

<ResponseField name="created_at" type="string">
  ISO 8601 timestamp of account creation
</ResponseField>

<ResponseField name="updated_at" type="string">
  ISO 8601 timestamp of last update
</ResponseField>

## Example

<RequestExample>

```typescript TypeScript
import { Rails } from "@rails/sdk";

const rails = new Rails({
  apiKey: process.env.RAILS_API_KEY
});

const account = await rails.accounts.retrieve("123e4567-e89b-12d3-a456-426614174000");

console.log("Account Type:", account.accountType);
console.log("Balance:", account.balance);
console.log("Status:", account.status);
```

```go Go
account, err := client.Accounts.Retrieve(ctx, "123e4567-e89b-12d3-a456-426614174000")
```

```java Java
var account = client.accounts().retrieve("123e4567-e89b-12d3-a456-426614174000");
```

```kotlin Kotlin
val account = client.accounts.retrieve("123e4567-e89b-12d3-a456-426614174000")
```

```csharp C#
var account = await client.Accounts.RetrieveAsync("123e4567-e89b-12d3-a456-426614174000");
```

</RequestExample>

<ResponseExample>

```json Success Response (200 OK)
{
  "id": "123e4567-e89b-12d3-a456-426614174000",
  "account_number": "ACC001234567",
  "account_type": "checking",
  "user_id": "456e7890-e89b-12d3-a456-426614174000",
  "balance": 1250.75,
  "currency": "USD",
  "status": "active",
  "organization_id": null,
  "environment": "production",
  "admin_user_id": null,
  "user_role": null,
  "created_at": "2024-01-15T10:35:00Z",
  "updated_at": "2024-01-15T15:20:00Z"
}
```

```json Error Response (404 Not Found)
{
  "error": {
    "code": "account_not_found",
    "message": "No account found with the provided ID"
  }
}
```

</ResponseExample>

## Error Codes

| Code | Description |
|------|-------------|
| `account_not_found` | Account ID does not exist |
| `unauthorized` | Invalid or missing API key |
| `invalid_uuid` | Invalid UUID format for account ID |